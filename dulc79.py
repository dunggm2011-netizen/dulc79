...
def init_user_state(chat_id):
    if chat_id not in user_states:
        user_states[chat_id] = {
            ...
            'last_sent_session': None,  # Lưu phiên đã gửi để tránh gửi trùng
            'prediction': None,
...
# ==========================================
# 🧠 THUẬT TOÁN 15 ENSEMBLE (GIỮ NGUYÊN)
# ==========================================
...
# ==========================================
# 🤖 VÒNG LẶP TỰ ĐỘNG (FETCH 5S, GỬI KHI CÓ PHIÊN MỚI)
# ==========================================
def prediction_loop(chat_id):
    ...
    while st['running']:
        try:
            ...
            # KIỂM TRA ĐÚNG/SAI + GỬI KHI CÓ PHIÊN MỚI
            if st.get('last_prediction') and last_result:
                ...
                # CHỈ GỬI KHI PHIÊN MỚI VÀ CHƯA GỬI
                if pred_session == last_session and pred_value and st.get('last_sent_session') != last_session:
                    ...
                    try:
                        ...
                        st['last_sent_session'] = last_session  # Đánh dấu đã gửi
                    except:
                        pass
            ...
            if history:
                ...
                # GỬI DỰ ĐOÁN CHO PHIÊN TIẾP THEO
...
def send_admin(message):
    ...
    markup.add(
        telebot.types.InlineKeyboardButton("🔑 Tạo Key", callback_data="admin_taokey"),
        telebot.types.InlineKeyboardButton("📋 Danh sách Key", callback_data="admin_listkey"),
        telebot.types.InlineKeyboardButton("📊 Thống kê bot", callback_data="admin_stats"),
        telebot.types.InlineKeyboardButton("🔄 Reset thống kê", callback_data="admin_reset"),
    )
